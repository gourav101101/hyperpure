...
export default function CommissionAnalytics() {
  ...
  const commissionRate = Number(analytics?.commissionRate ?? 0);
  const totalOrders = Number(analytics?.totalOrders ?? 0);
  const payoutStats = analytics?.payoutStats ?? { pending: 0, processing: 0, completed: 0 };
  ...
  const maxCommission = Math.max(1, ...chartData.map((d: any) => Number(d?.commission ?? 0)));
    ...
    <>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 text-white">
          ...
          <p className="text-3xl font-bold">Rs. {totalCommission.toFixed(0)}</p>
          <p className="text-xs opacity-75 mt-2">{totalOrders} orders</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-6 text-white">
          ...
          <p className="text-3xl font-bold">Rs. {Number(payoutStats.pending ?? 0).toFixed(0)}</p>
        ...
        <div className="bg-white rounded-xl border p-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg">
              ...
              <span className="font-bold text-yellow-700">Rs. {Number(payoutStats.pending ?? 0).toFixed(0)}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
              ...
              <span className="font-bold text-blue-700">Rs. {Number(payoutStats.processing ?? 0).toFixed(0)}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
              ...
              <span className="font-bold text-green-700">Rs. {Number(payoutStats.completed ?? 0).toFixed(0)}</span>
            </div>
          </div>
        </div>
            ...
            {topSellers.slice(0, 5).map((seller: any, idx: number) => (
              <div key={seller.sellerId} className="flex items-center justify-between p-2 hover:bg-gray-50 rounded">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-sm">{String(seller.sellerId || "").slice(-6)}</p>
                    <p className="text-xs text-gray-500">{Number(seller.orders ?? 0)} orders</p>
                  </div>
                </div>
                <span className="font-bold text-green-600">Rs. {Number(seller.commission ?? 0).toFixed(0)}</span>
              </div>
            ))}
            {topSellers.length === 0 && (
              <div className="text-sm text-gray-500">No seller data for this period.</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border p-6">
        ...
        {chartData.length === 0 ? (
          <div className="text-sm text-gray-500">No chart data for this period.</div>
        ) : (
          <div className="h-64 flex items-end gap-2">
            {chartData.map((data: any) => {
              const commission = Number(data?.commission ?? 0);
              const height = Math.round((commission / maxCommission) * 100);
              return (
